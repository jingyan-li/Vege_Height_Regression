...
if __name__ == "__main__":
    DTYPE = "test"
    dset = h5py.File(f"../data/dataset_rgb_nir_{DTYPE}.hdf5","r")
    CLOUDNAME = "CLD_"
    LAYERNAMES = ["INPT","NIR"]
    IDX = np.arange(1,6) if DTYPE=="train" else [0,5]

    # Copy GT to h5py
    with h5py.File(f"../data/data_{DTYPE}_rgbReduced_delBlankRotations.hdf5","w") as f:
        if "GT" not in f.keys():
            print("writing GT")
    ...
    with h5py.File(f"../data/data_{DTYPE}_rgbReduced_delBlankRotations.hdf5", "a") as f:
        if (LAYERNAMES[0] not in f.keys()) or (LAYERNAMES[1] not in f.keys()):
            ...
            for i in tqdm(range(len(IDX))):
                rgb_layer_name = f"{LAYERNAMES[0]}_{IDX[i]}"
                nir_layer_name = f"{LAYERNAMES[1]}_{IDX[i]}"
                cloud = dset[f"{CLOUDNAME}{IDX[i]}"]
                rgb = dset[rgb_layer_name]
                nir = dset[nir_layer_name]
                # Remove cloud, take average
                rgb_avg, nir_avg = remove_cloud(cloud, rgb, nir)
                del cloud, rgb, nir
                rgb_avg_all[i] = rgb_avg
                nir_avg_all[i] = nir_avg
            # Save into h5py
