...
if __name__=="__main__":
    DTYPE = "test"
    af_dset = h5py.File(f"../data/data_{DTYPE}_rgbReduced_delBlankRotations.hdf5", "r")
    print(af_dset.keys())
    rgb_avg_all = np.zeros(af_dset["INPT"].shape)
    nir_avg_all = np.zeros(af_dset["NIR"].shape)
    for i in tqdm(range(af_dset["INPT"].shape[0])):
        rgb_avg_all[i] = normalize(af_dset["INPT"][i]).data
        nir_avg_all[i] = normalize(af_dset["NIR"][i]).data
    print("Writing to file...")
    # Save to h5py
    with h5py.File(f"../data/data_{DTYPE}_rgbReduced_delBlankRotations_Standardized.hdf5", "a") as f:
        # Save into h5py
